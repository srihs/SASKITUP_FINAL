            ...
            with connection.cursor() as cursor:
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS authentication_user (
                        ...
                        is_active_sales_rep BOOLEAN NOT NULL DEFAULT 1,
                        last_login_ip VARCHAR(39) NULL,
                        created_at DATETIME NOT NULL,
                        updated_at DATETIME NOT NULL
                    )
                """)
            print("✅ Created authentication_user table manually")

            # Try again to create users
            return create_demo_users()

        except Exception as table_error:
            print(f"❌ Could not create table: {table_error}")
            print("\n⚠️  Please run: python manage.py migrate --run-syncdb")
            return False
